...
main() {
  ...
  printf "${BLUE}Cloning Joy...${NORMAL}\n"
  ...
  env git clone --depth=1 https://geekhacks@dev.azure.com/geekhacks/Joy/_git/Joy "$JOY" || {
    printf "Error: git clone of Joy repo failed\n"
    exit 1
  }

  # Switch to the newly cloned directory and update packages
  printf "${BLUE}Installing Node dependencies...${NORMAL}\n"
  cd $JOY
  npm i

  # Ensure the bootstrap script is executable
  printf "${BLUE}Creating executable...${NORMAL}\n"
  chmod +x ~/.joy/utility/joy.sh
  sudo ln -s ~/.joy/utility/joy.sh /usr/local/bin/joy
  
  printf "${BLUE}Try joy help or joy version to confirm installation${NORMAL}\n"
}
